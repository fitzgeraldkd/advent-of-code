...
class Year2015Day22(BaseSolution):
    ...
    def _parse_inputs(self):
        hp, damage = super()._parse_inputs()
        return {
            "hp": hp,
            "damage": damage,
        }

    def run_effects(self, game_state):
    ...
    def new_turn(self, old_game_state, spell, queue, checked_states, hard_mode=False):
        ...
        armor = self.run_effects(game_state)
        ...
        for next_spell in SPELLS:
            is_effect_active = (
                next_spell["effect"] is not None
                and next_spell["effect"] in game_state["effects"]
                and game_state["effects"][next_spell["effect"]] > 1
            )
            if is_effect_active:
                continue
            ...
            if json.dumps(game_state) not in checked_states:
                queue.add_item(
                    {"game_state": game_state, "spell": next_spell},
                    game_state["mana_spent"] + next_spell["cost"],
                )
    ...
    def part_1(self):
        ...
        for spell in SPELLS:
            queue.add_item(
                {"game_state": game_state, "spell": spell},
                game_state["mana_spent"] + spell["cost"],
            )

        while not queue.is_empty():
            item = queue.pop()
            winning_game_state = self.new_turn(
                item["game_state"], item["spell"], queue, checked_states
            )
    ...
    def part_2(self):
        ...
        while not queue.is_empty():
            item = queue.pop()
            winning_game_state = self.new_turn(
                item["game_state"], item["spell"], queue, checked_states, hard_mode=True
            )
            if winning_game_state:
                return winning_game_state["mana_spent"]
